...
/*
* RankerFull - Modified version of Ranker that keeps FULL matchup data
* This version doesn't limit to top 5 matchups/counters - it keeps ALL matchups
* Used for pogo_teambuilder app to have complete matchup matrix
*/

var RankerMasterFull = (function () {
    var instance;
    ...
    function createInstance() {
		...
		function rankerObject(){
			...
			this.rank = function(league, scenario){
				...
				// MODIFIED FOR FULL MATCHUPS: Keep ALL matches, clean up unnecessary data
				
				var matches = rankings[i].matches;

				// Clean up unnecessary data from each match to reduce file size
				...
				// Sort all matches by rating (descending) for easier consumption
				rankings[i].matches.sort((a,b) => (a.rating > b.rating) ? -1 : ((b.rating > a.rating) ? 1 : 0));
				
				// Create summary matchups and counters for backward compatibility (top 5 each)
				var matchupCount = Math.min(5, rankings[i].matches.length);
				
				// Top 5 wins (rating > 500)
				rankings[i].matchups = rankings[i].matches
					.filter(m => m.rating > 500 || cup.name == "bidoof")
					.slice(0, matchupCount)
					.map(m => ({opponent: m.opponent, rating: m.rating, opRating: m.opRating}));
				
				// Top 5 losses (rating < 500), sorted by rating ascending
				rankings[i].counters = rankings[i].matches
					.filter(m => m.rating < 500 || cup.name == "bidoof")
					.slice(-matchupCount)
					.reverse()
					.map(m => ({opponent: m.opponent, rating: m.rating}));
				
				// Keep ALL matches in a new field "allMatches" for complete data
				rankings[i].allMatches = rankings[i].matches.map(m => ({
					opponent: m.opponent, 
					rating: m.rating,
					opRating: m.opRating
				}));
				
				// Keep original matches field for tools that might expect it
				// But clean it to avoid duplication

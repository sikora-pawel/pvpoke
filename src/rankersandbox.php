...
<div class="section league-select-container">
	...
	<div style="margin: 20px 0; text-align: center;">
		<button class="simulate">Simulate Selected Cup</button>
		<button class="simulate-all" style="margin-left: 10px; background: #e67e22;">🚀 Generate All Cups (Batch)</button>
	</div>
...
function processBatch() {
	if(batchCurrentIndex >= batchQueue.length) {
		...
		console.log("📦 Generated overall and consistency rankings for " + batchQueue.length + " cups");
		console.log("");
		...
		console.log("  git commit -m \"Regenerate overall and consistency rankings for all active cups\"");
...
function waitForCompletion(item) {
	...
	var completionHandler = function(event, data) {
		...
		$(document).off('overallRankingComplete', completionHandler);
	...
	$(document).on('overallRankingComplete', completionHandler);
...
$(document).ready(function(){
	...
	$('.simulate-all').on('click', function() {
		...
		var confirm = window.confirm(
			'This will generate overall and consistency rankings for ALL ' + allCupsData.length + ' cups.\n\n' +
			'⚠️ IMPORTANT: Make sure you have already generated the 5 base categories (leads, closers, switches, chargers, attackers) for all cups first!\n\n' +
			'Estimated time: ' + Math.round(allCupsData.length * 1) + ' minutes\n\n' +
		...
		console.log("");
		console.log("🚀 STARTING BATCH PROCESSING FOR OVERALL/CONSISTENCY");
